...
class _ClientPreferencesScreenState extends State<ClientPreferencesScreen> {
  ...
  Future<void> _finish() async {
    ...
    try {
      ...
      await prefs.setStringList('selected_sports', widget.selectedSports);
      await prefs.setDouble('budget_min', _minPrice);
      await prefs.setDouble('budget_max', _maxPrice);
      await prefs.setString('rating_preference', _ratingPreference ?? '');
      await prefs.setString('location_preference', _locationPreference ?? '');
      await prefs.setString('selected_city', _selectedCity ?? '');
      await prefs.setString('selected_area', _selectedArea ?? '');
      await prefs.setString('coach_gender', _coachGender ?? '');
      await prefs.setString('coach_age_range', _coachAgeRange ?? '');
      await prefs.setBool('certified_only', _certifiedOnly);

      if (!mounted) return;

      // ✅ Navigate to CLIENT LOGIN screen instead of home
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(
          builder: (_) => const LoginScreen(userType: 'client'),
        ),
            (route) => false,
      );
    } catch (e) {
