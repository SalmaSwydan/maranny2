...
// ✅ FIX: removed duplicate import — was imported twice with different relative paths
...
class _ListItemScreenState extends State<ListItemScreen> {
  ...
  Future<String?> _saveImageToAppStorage() async {
    ...
    try {
      ...
      final marketplaceDir =
      Directory('${dir.path}/marketplace_images');
      if (!await marketplaceDir.exists()) {
        await marketplaceDir.create(recursive: true);
      }
      final savedPath =
          '${marketplaceDir.path}/product_${DateTime.now().millisecondsSinceEpoch}.jpg';
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 6),
                    _buildTextField(controller: _priceController, hint: '0', keyboardType: TextInputType.number),
                    const SizedBox(height: 16),
                    ...
                    const SizedBox(height: 6),
                    _buildTextField(controller: _sellerNameController, hint: 'Store or seller name'),
                    const SizedBox(height: 12),
                    _buildTextField(controller: _sellerPhoneController, hint: 'Your phone number (e.g. +20 100 123 4567)', keyboardType: TextInputType.phone),
                    const SizedBox(height: 12),
                    _buildTextField(controller: _sellerLocationController, hint: 'Your location (e.g. Cairo, Egypt)'),
                    const SizedBox(height: 20),
                    Container(
